...
//复制ueditor
gulp.task('copy:ueditor', function (done) {
    gulp.src(['src/ueditor/**/*']).pipe(gulp.dest('dist/ueditor')).on('end', done);
});
//复制css
gulp.task('copy:css', function (done) {
    gulp.src(['src/css/lib/**/*.css']).pipe(gulp.dest('dist/css/lib')).on('end', done);
});
//压缩合并css, css中既有自己写的.less, 也有引入第三方库的.css
gulp.task('sass',['copy:css'], function (done) {
...
//       .pipe(minifycss())                		       //执行压缩
         ...
         gulp.src(['src/css1.0/*.css','src/css1.0/*.scss'])       		  //压缩的文件
        .pipe(sourcemaps.init())
        .pipe(postcss([ autoprefixer() ]))
        .pipe(sass().on('error', sass.logError)) //解析sass
        .pipe(concat('app.min1.0.css'))
        .pipe(sourcemaps.write('./'))				  //map映射文件
        .pipe(gulp.dest('dist/css'))		        //输出文件夹
        .on('end', done);
});
...
//开发
gulp.task('dev', ['connect', 'copy:fonts', 'copy:images','copy:ueditor', 'sass', 'build-js', 'watch', 'open']);
