		 ...
		 */
		 paypay: function() {
		 	...
		 	var param = {
				...
				payAmount: 0.01, //_this.orderinfo.payAmount,
				balanceAmount: 0,
				rooCur: 0,
		 ...
		 */
		 changebalan: function() {
		 	...
		 	if(_this.balanceAmount>0){
		 		if(_this.balanceAmount<= 0.01){
		 			_this.balanceAmount = 0.1
		 		};		 				 		
		 		if(String(_this.balanceAmount).split(".")[1]){
					if(String(_this.balanceAmount).split(".")[1].length>2){console.log(222)
				 // 		Message({
			 	// 			type: 'error',
			 	// 			message:'只能保留2位小数'
			 	// 		});
			 			_this.balanceAmount = parseFloat(_this.balanceAmount).toFixed(2);
			 	// 		return ;
				 	};
		 		};
		 		if(_this.balanceAmount >= _this.orderinfo.payAmount ){
		 			_this.orderinfo.balanceAmount = _this.orderinfo.payAmount;
		 		};		 		
		 ...
		 changeKangarooCurrency: function() {
		 	var _this = this;
		 	if(_this.rooCur>0){
		 		if(_this.rooCur<= 0.01){
		 			_this.rooCur = 0.1
		 		};		 				 		
		 		if(String(_this.rooCur).split(".")[1]){
					if(String(_this.rooCur).split(".")[1].length>2){console.log(222)
				 // 		Message({
			 	// 			type: 'error',
			 	// 			message:'只能保留2位小数'
			 	// 		});
			 			_this.rooCur = parseFloat(_this.rooCur).toFixed(2);			 	
				 	};
		 		};
		 		if(_this.rooCur >= _this.orderinfo.payAmount ){
		 			_this.orderinfo.rooCur = _this.orderinfo.payAmount;
		 		};
		 		...
		 		LF.net.getJSON("/integral/paymentStatus", {
		 			...
		 			if (res.code == "000") {
		 				if (res.data.payStatus) {
		 					...
		 					LF.window.openWindow("/app/pages/personal/payOrdersSuccess.html?orderId=" + param['order'], "_self");
		 				}
		 			} else {
		 				clearInterval(t);
		 				Message({
	 ...
	 */
	 mounted() {
	 	...
	 	if (LF.cookie.get("tokenId") == null || LF.cookie.get("tokenId") == '') {
			...
			LF.net.getJSON("/merchant/order/detail", {
				tokenId: LF.cookie.get("tokenId"),
				orderId: param.order
			}, res => {
				_this.payLoading = false;
				if (res.code == "000") {				console.log(res.data,555)
					_this.orderinfo = res.data;
					_this.needPayAmount = res.data.totalAmount
				}
			}, res => {
				console.log("error：" + JSON.stringify(res));
			});
